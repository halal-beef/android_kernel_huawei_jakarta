...
static int g_fix_coveriry_power_misc;
...
void power_misc_handler(void *arg)
{
	...
	//mutex_lock(&sdd->lock); // delete by MTK ALPS04212465, no use lock, will hang fuelgauged
...
static int power_misc_routine_thread(void *arg)
{
	...
	while (1) {
		...
		if (g_fix_coveriry_power_misc == EXITLOOP)
			return 0;
	}
