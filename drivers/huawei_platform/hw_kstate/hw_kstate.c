...
static int send_to_user(struct ksmsg *msg)
{
	...
	//pr_err("hw_kstate %s: sizeKmsg: %d, length: %d, len: %d\n", __func__, sizeof(struct ksmsg), msg->length, len);
...
static void recv_from_user(struct sk_buff *skb)
{
	...
	if (tmp_skb->len >= NLMSG_SPACE(0)) {
		...
		pr_err("hw_kstate %s: len: %d\n", __func__, len);
		data = (struct ksmsg*)NLMSG_DATA(nlh);
		pr_err("hw_kstate %s: sizeKmsg: %d, length: %d, len: %d\n", __func__, sizeof(struct ksmsg), data->length, len);
		if (len >= (sizeof(struct ksmsg) + data->length)) {
			pr_err("hw_kstate %s: good sizeKmsg: %d, length: %d, len: %d\n", __func__, sizeof(struct ksmsg), data->length, len);
		}
		if (len < 0) {
			pr_err("hw_kstate %s: minus sizeKmsg: %d, length: %d, len: %d\n", __func__, sizeof(struct ksmsg), data->length, len);
		}
		if ((len >= (sizeof(struct ksmsg) + data->length))
			&& (len < 1024 * 1024)
			&& (len > 0)
			&& (tmp_skb->len >= nlh->nlmsg_len)) {
			pr_err("hw_kstate %s: bad len: %d\n", __func__, len);
