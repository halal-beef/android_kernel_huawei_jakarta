...
#define SMARTPA_VENDOR 2
#define SMARTPA_MODEL "tas2557"
	...
	const struct i2c_device_id *pID)
{
	...
	if (gpio_is_valid(pTAS2557->mnResetGPIO)) {
		nResult = devm_gpio_request_one(pTAS2557->dev,
				pTAS2557->mnResetGPIO, GPIOF_OUT_INIT_LOW,
				"TAS2557-RESET");
		if (nResult < 0) {
	...
	if (gpio_is_valid(pTAS2557->mnGpioINT)) {
		nResult = devm_gpio_request_one(pTAS2557->dev,
				pTAS2557->mnGpioINT, GPIOF_DIR_IN,
				"TAS2557-IRQ");
		if (nResult < 0) {
		...
		nResult = devm_request_threaded_irq(pTAS2557->dev,
				pTAS2557->mnIRQ, tas2557_irq_handler,
	...
	nResult = smartpakit_set_info(&vendor_info);
	if (nResult) {
		pr_err("%s: set info err %d", __func__, nResult);
		goto err;
	}
#ifdef CONFIG_TAS2557_MISC
...
late_initcall_sync(tas2557_init);

static void __exit tas2557_exit(void)
{
