...
static inline int occupy_free_smc_in_entry(TC_NS_SMC_CMD *cmd)
{
	...
	for (i=0; i<MAX_SMC_CMD; i++) {
		if (hm_test_bit(i, cmd_data->in_bitmap))
			continue;
		...
		isb();
		wmb();
		hm_set_bit(i, cmd_data->in_bitmap);
		hm_set_bit(i, cmd_data->doing_bitmap);
		idx = i;
		break;
	}
	...
	isb();
	wmb();
	hm_clear_bit(idx, cmd_data->doing_bitmap);
	release_smc_buf_lock(&cmd_data->smc_lock);

	return idx;
clean:
	acquire_smc_buf_lock(&cmd_data->smc_lock);
	hm_clear_bit(i, cmd_data->in_bitmap);
	hm_clear_bit(i, cmd_data->doing_bitmap);
	release_smc_buf_lock(&cmd_data->smc_lock);
...
{
	acquire_smc_buf_lock(&cmd_data->smc_lock);
	if (!hm_test_bit(idx, cmd_data->out_bitmap)) {
	...
	if (cmd_data->out[idx].cmd_id == SS_AGENT_MSG
		...
		hm_clear_bit(idx, cmd_data->in_bitmap);
		hm_clear_bit(idx, cmd_data->doing_bitmap);
		if (usage)
			*usage = clear;
	}
	hm_clear_bit(idx, cmd_data->out_bitmap);
...
static inline int copy_shadow_task_agent_cmd(TC_NS_SMC_CMD *copy)
{
	...
	acquire_smc_buf_lock(&cmd_data->smc_lock);
	for (i=0; i<MAX_SMC_CMD; i++) {
		if (hm_test_bit(i, cmd_data->out_bitmap)
			...
			hm_clear_bit(i, cmd_data->in_bitmap);
			hm_clear_bit(i, cmd_data->doing_bitmap);
			hm_clear_bit(i, cmd_data->out_bitmap);
...
void show_cmd_bitmap(void)
{
	...
	for (idx = 0; idx < MAX_SMC_CMD; idx++) {
		if (hm_test_bit(idx, cmd_data->in_bitmap)) {
			bitmap[idx] = '1';
			cmd_in[in++] = idx;
		} else {
			bitmap[idx] = '0';
	...
	TCERR("in_bitmap: %s\n", bitmap);
	for (idx = 0; idx < MAX_SMC_CMD; idx++) {
		if (hm_test_bit(idx, cmd_data->doing_bitmap)) {
			bitmap[idx] = '1';
		} else {
			bitmap[idx] = '0';
		}
					...
					smc_cmd_ret_t *secret)
{
	...
	u32 r3 = 0;
	u32 r4 = 0;
	u32 ret = 0, exit_reason = 0, ta = 0, target = 0;
...
retry:
	...
	if (secret && (SMC_OPS_SCHEDTO == ops || SMC_OPS_START_FIQSHD == ops))
		r4 = secret->target;
	tlogd("[cpu %d] start to send r0=%llx r1=%llx r2=%llx r3=%llx r4=%llx\n",
...
{
	u32 r0 = TSP_REQUEST;
	u32 r1 = SMC_OPS_START_SHADOW;
	u32 r2 = current->pid;
	u32 r3 = 0;
	u32 r4 = *(u64 *)arg;
	u32 ret = 0, exit_reason = SMC_EXIT_MAX, ta = 0, target = 0;
	int n_preempted = 0, n_idled = 0, ret_val = 0;
	struct pending_entry *pe;
	TC_NS_SMC_CMD cmd = {0};
	bool agent_request = false;
	...
	isb();
	wmb();
	tlogd("%s: [cpu %d] r0=%llx r1=%llx r2=%llx r3=%llx r4=%llx\n", __func__,
		raw_smp_processor_id(), r0, r1, r2, r3, r4);
	if (hisi_secs_power_on()) {
		tloge("hisi_secs_power_on failed\n");
		kfree(arg);
