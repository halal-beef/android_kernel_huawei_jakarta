...
/* Global varible                                                       */
/*---------------------------------------------------------------------------*/
extern  int mdss_dsi_write_cmds(struct mipi_panel_info *mipi,
...
int target_backlight_ctrl(struct backlight *bl, uint8_t enable)
{
	...
	else if(target_get_pmic() == PMIC_IS_PMI632 && bl->bl_interface_type == BL_PWM)
	{
						...
						struct msm_panel_info *pinfo)
{
	...
	if (enable)
    {
		...
		dprintf(INFO, "%s lcd %d \n", __func__, (int)tp_reset_gpio.pin_id);
		...
		if (lcdkit_config.lcd_misc->tp_lcd_reset_sync) {
			lcdkit_gpio_cfg(tp_reset_gpio,0,GPIO_STATE_HIGH);
		}
		dprintf(INFO,"the tp_reset_gpio is %d\n", tp_reset_gpio.pin_id);
		/* reset */
...
int target_power_on(struct msm_panel_info *pinfo)
{
	...
	{	
		...
		else
		{
			...
			lcdkit_bias_ic_init(mdss_dsi_get_panel_data().lcd_bias_ic_info.lcd_bias_ic_list, mdss_dsi_get_panel_data().lcd_bias_ic_info.num_of_lcd_bias_ic_list);
