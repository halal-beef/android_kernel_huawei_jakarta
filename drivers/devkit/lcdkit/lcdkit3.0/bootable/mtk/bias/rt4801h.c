...
static void rt4801h_set_bias_status(const char *compname, const char *status)
{
	int ret;
	int offset;
	...
	ret = fdt_setprop_string(g_fdt, offset, (const char *)"status",
		(const void *)status);
	if (ret) {
...
int rt4801h_bias_recognize(void)
{
	...
	if (ret < 0) {
		rt4801h_set_bias_status(DTS_COMP_RT4801H, "disabled");
		LCD_KIT_ERR("rt4801h is not right bias\n");
	} else {
		lcd_kit_bias_register(&bias_ops);
		rt4801h_set_bias_status(DTS_COMP_RT4801H, "okay");
