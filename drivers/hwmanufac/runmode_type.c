...
#define RUNMODE_TYPE_ID         "androidboot.swtype"
#define HWLOG_TAG               HW_RUNMODE_TYPE
HWLOG_REGIST();
...
static int __init early_parse_runmode_cmdline(char *p)
{
	if (p) {
		...
		hwlog_err("runmode is %s, runmode_type = %d\n", p, runmode_type);
	}
	return 0;
}

early_param(RUNMODE_TYPE_ID, early_parse_runmode_cmdline);
