...
#define DUMP_LOG_TIMEOUT 10
#define LONG_PRESS_TIMEOUT 6
...
static int dump_last_log(void *arg)
{
	...
	ret = wait_event_interruptible_timeout(dump_log_wait, dump_thread == false, DUMP_LOG_TIMEOUT * HZ);
...
void kpd_pmic_pwrkey_hal(unsigned long pressed)
{
	...
	if (pressed) {
		long_press_dump_timer.expires = jiffies + LONG_PRESS_TIMEOUT * HZ;
		...
		if (!timer_pending(&long_press_dump_timer)) {
			...
			handle_thread = kthread_create(dump_last_log,
				NULL, "dump_log");
