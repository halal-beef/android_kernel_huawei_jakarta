...
static void gc5025_gcore_update_dd(void)
{
	...
	/* write SRAM for init */
	(void)imgsensor_sensor_write_setting(&imgsensor, &otp_sram_write_init_setting_array);
	...
	for (i = 0; i < gc5025_otp_info.dd_cnt; i++) {
		...
		LOG_INF("GC5025_OTP dd[%d] = (%d, %d);dd_type = %d\n",
			i, ((temp_val1 & 0x0f) << 8) + temp_val0, (temp_val2 << 4) + ((temp_val1 & 0xf0) >> 4), gc5025_otp_info.dd_param[i].t);
	}
