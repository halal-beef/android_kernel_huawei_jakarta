...
#define  ABANDON_INVALIED_VDDP_VALUE 3450000
                                                        ...
                                                        u32 *measure)
{
	...
	mt_dbg(chg_data->dev,"%s: post_aicc = (%d, %d, %d)\n", __func__, start, stop , step);
...
static int __mt6370_run_aicl(struct mt6370_pmu_charger_data *chg_data)
{
	...
	aicl_vth = mivr + 200000; // by hardware, unit: uv
	...
	ret = mt6370_post_aicl_measure(chg_data->chg_dev,
					aicr,aicr + 200000, 50000, &aicr);
	if (ret < 0)
		goto out;
...
static int mt6370_get_ts_vbat_adc(struct charger_device *chg_dev)
{
	int ret = 0;
	int CHG_VDDP = 0, TS_VBAT = 0;

	struct mt6370_pmu_charger_data *chg_data =dev_get_drvdata(&chg_dev->dev);
	if(chg_data)
	{
		ret = mt6370_get_adc(chg_data, MT6370_ADC_CHG_VDDP, &CHG_VDDP);
		if( CHG_VDDP < ABANDON_INVALIED_VDDP_VALUE )
			ret = mt6370_get_adc(chg_data, MT6370_ADC_CHG_VDDP, &CHG_VDDP);
		ret = mt6370_get_adc(chg_data, MT6370_ADC_TS_BAT, &TS_VBAT);
		dev_info(chg_data->dev, "%s: CHG_VDDP = %d, TS_VBAT = %0d\n",__func__, CHG_VDDP, TS_VBAT);
		return TS_VBAT;
	}
	return 0;
}
...
static struct charger_ops mt6370_chg_ops = {
	...
	.get_ts_vbat_adc = mt6370_get_ts_vbat_adc,
