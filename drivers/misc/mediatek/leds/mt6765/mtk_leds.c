...
int pmic_freqsel_array[] = { 249, 499, 749, 999, 1249, 1499, 1749, 1999 };
...
static bool g_red_led_timer_init_flag = false;
static bool g_should_stop_timer_flag = false;
static bool g_red_led_is_on = false;
static ktime_t g_time_on;
static int g_red_led_blink_on = 0;
static int g_red_led_blink_off = 0;
...
static enum hrtimer_restart red_led_blink_func(struct hrtimer *timer)
{
	if (!g_should_stop_timer_flag) {
		if (!g_red_led_is_on) {
			...
			g_time_on = ktime_set(g_red_led_blink_off / MSEC_PER_SEC, (g_red_led_blink_off % MSEC_PER_SEC) * NSEC_PER_MSEC);
			hrtimer_start(&g_red_led_blink_timer, g_time_on, HRTIMER_MODE_REL);
		}
	}
