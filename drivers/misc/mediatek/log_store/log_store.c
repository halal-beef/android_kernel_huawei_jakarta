...
void log_store_to_emmc(void)
{
	...
	if (file_size <= EXPDB_LOG_SIZE) {
		pr_notice("file size is too small");
		set_fs(fs);
		return;
	}
	...
	sys_read(fd, (char *)&header, sizeof(header));
	if (header.sig != LOG_EMMC_SIG) {
	...
	while (kmsg_dump_get_line_nolock(&dumper, true, buff, LOG_BLOCK_SIZE, &len)) {
		if (header.offset + len + LOG_BLOCK_SIZE > EXPDB_LOG_SIZE)
			header.offset = 0;
		sys_lseek(fd, file_size - EXPDB_LOG_SIZE + header.offset, 0);
		size = sys_write(fd, buff, len);
		if (size < 0)
			pr_notice_once("write expdb failed: %ld\n", size);
		else
			header.offset += size;
		memset(buff, 0, LOG_BLOCK_SIZE);
	}
	...
	sys_lseek(fd, file_size - LOG_BLOCK_SIZE, 0);
	sys_write(fd, (char *)&header, sizeof(header));
	sys_close(fd);
	set_fs(fs);
