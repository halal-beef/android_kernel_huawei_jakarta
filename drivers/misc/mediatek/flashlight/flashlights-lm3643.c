...
#define LM3643_FLASH_TIMEOUT   0x0C

#define LM3643_REG_FLAG1 0x0A
#define LM3643_REG_FLAG2 0x0B

#define LM3643_REG_CHIPID 0x0C
#define SY7806_CHIP_ID 0x18
#define LM3643_CHIP_ID 0x10
#define MASK_ID 0x38
...
#define LM3643_LEVEL_TORCH 9
#define LM3643_LEVEL_TORCH_FRONT 12
...
static const unsigned char sy7806_torch_level[LM3643_LEVEL_NUM] = {
	0x11, 0x23, 0x35, 0x46, 0x58, 0x6A, 0x7C, 0x7F, 0x7F, 0x7F,
	0x7F, 0x7F, 0x7F, 0x7F, 0x7F, 0x7F, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00
};

static const unsigned char sy7806_torch_level_front[LM3643_LEVEL_NUM] = {
	0x11, 0x23, 0x35, 0x46, 0x47, 0x4A, 0x4F, 0x50, 0x54, 0x5A,
	0x5F, 0x68, 0x70, 0x78, 0x7A, 0x7F, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00
};

static const unsigned char lm3643_torch_level_front[LM3643_LEVEL_NUM] = {
	0x08, 0x10, 0x19, 0x21, 0x22, 0x23, 0x26, 0x26, 0x28, 0x2B,
	0x2D, 0x32, 0x35, 0x39, 0x3A, 0x3D, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00
};

static const unsigned char lm3643_torch_level[LM3643_LEVEL_NUM] = {
	0x08, 0x10, 0x19, 0x21, 0x2A, 0x33, 0x3B, 0x43, 0x4B, 0x58,
	0x5F, 0x68, 0x70, 0x78, 0x7F, 0x7F, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00
};

static const unsigned char lm3643_flash_level[LM3643_LEVEL_NUM] = {
	0x01, 0x03, 0x05, 0x08, 0x0A, 0x0C, 0x0E, 0x10, 0x12, 0x14,
	0x16, 0x19, 0x1B, 0x1D, 0x1F, 0x21, 0x25, 0x2A, 0x2E, 0x32,
	0x36, 0x3B, 0x3F, 0x43, 0x47, 0x4C, 0x50, 0x54, 0x59, 0x5D,
	0x61, 0x65
};
...
static int lm3643_is_torch_front(int level)
{
	if (level >= LM3643_LEVEL_TORCH_FRONT)
		return -1;
...
static int lm3643_verify_level_front(int level)
{
	if (level < 0)
		level = 0;
	else if (level >= LM3643_LEVEL_TORCH_FRONT)
		level = LM3643_LEVEL_TORCH_FRONT - 1;
...
static int lm3643_enable_ch2(void)
{
	...
	reg = LM3643_REG_ENABLE;
	if (!lm3643_is_torch_front(lm3643_level_ch2)) {
		/* torch mode */
...
static int lm3643_set_level_ch1(int level)
{
	...
	pr_info("ch1 flashlight brightness level = %d,chip_id = 0x%x", level, chip_id);

	/* set torch brightness level */
	reg = LM3643_REG_TORCH_LEVEL_LED1;
	if (chip_id == SY7806_CHIP_ID)
...
static int lm3643_set_level_ch2(int level)
{
	...
	level = lm3643_verify_level_front(level);
	pr_info("ch2 flashlight brightness level = %d,chip_id = 0x%x", level, chip_id);

	/* set torch brightness level */
	reg = LM3643_REG_TORCH_LEVEL_LED2;
	if (chip_id == SY7806_CHIP_ID)
		val = sy7806_torch_level_front[level];
	else
		val = lm3643_torch_level_front[level];
...
static unsigned int lm3643_chip_init_read(void)
{
	int retry;
	int val;
	...
	for (retry = 0; retry < RETRY_TIMES; retry++) {
		val = lm3643_read_reg(lm3643_i2c_client, LM3643_REG_CHIPID);
		if (val < 0) {
			pr_err("read CHIP ID failed, ignore, retry:%d\n", retry);
			continue;
		}
		chip_id = (unsigned int)val & MASK_ID;
		if ((chip_id == SY7806_CHIP_ID) || (chip_id == LM3643_CHIP_ID)) {
			pr_info("flashlight id:0x%x\n", chip_id);
			break;
		}
	}

	if (retry == RETRY_TIMES)
		pr_info("read CHIP ID failed, 0x%x\n", chip_id);
...
int lm3643_init(void)
{
	...
	if (chip_id == SY7806_CHIP_ID)
		val = LM3643_TORCH_RAMP_TIME | SY7806_FLASH_TIMEOUT;
	else
