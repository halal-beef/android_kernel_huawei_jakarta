...
{
	int i = 0;
	unsigned int opptab_size;

	g_opp_table = kzalloc((num) * sizeof(*freqs), GFP_KERNEL);
	g_opp_table_default = kzalloc((num) * sizeof(*freqs), GFP_KERNEL);

	opptab_size = (num) * sizeof(*freqs);
	if (g_opp_table == NULL) {
		gpufreq_pr_debug("g_opp_table: mem allocation failed (%u)",
		opptab_size);
		return;
	}
	...
	if (g_opp_table_default == NULL) {
		kfree(g_opp_table);
		gpufreq_pr_debug("g_opp_table_default: mem allocation failed (%u)",
		opptab_size);
		return;
	}
