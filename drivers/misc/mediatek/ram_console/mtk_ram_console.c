...
unsigned long get_last_kmsg_pstore(char *dst, unsigned long length)
{
	...
	while ((size = psi->read(&id, &type, &count, &time, &buf, &compressed,
			...
			{
				if(size > length){
					pr_notice("ram console buffer size %zx!\n", size);
					memcpy(dst, buf+size-length, length);
					log_length = length;
					}
					else{
					memcpy(dst, buf, size);
...
out:
	...
	pr_notice("ram console buffer log_length %lu!\n", log_length);
	return log_length;
}
#endif
