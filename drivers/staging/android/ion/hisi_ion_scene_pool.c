...
static unsigned int scene_pool_orders[] = {4, 1, 0};

static const int scene_pool_num_orders = ARRAY_SIZE(scene_pool_orders);

static void ion_scene_pool_all_free(struct ion_scene_pool *pool, gfp_t gfp_mask, int nr_to_scan);

static void ion_scene_destroy_pools(struct ion_page_pool **pools)
{
	int i;
...
static int ion_scene_create_pools(struct ion_page_pool **pools, bool cached)
{
	int i;
	...
	for (i = 0; i < scene_pool_num_orders; i++) {
		...
		if (scene_pool_orders[i] >= 1)
			gfp_flags = (gfp_flags | __GFP_NOWARN | __GFP_NORETRY) & ~__GFP_RECLAIM;

		pool = ion_page_pool_create(gfp_flags, scene_pool_orders[i], cached);
		if (!pool)
					    ...
					    struct list_head *page_list)
{
	...
	pr_info("ion_scene_pool_allocate_pages have %d need %d kb\n",ion_scene_pool_total_pages(pool)*4, size*4/PAGE_SIZE);//only for debug

	if (ion_scene_pool_total_pages(pool) <= 0) {
		pr_info("ion_scene_pool_allocate_pages no page\n");
		return 0;
	}
					...
					struct ion_scene_pool *pool)
{
	...
	if (sched_ret < 0)
		pr_debug("%s: sched_setaffinity ret %d!\n", __func__, sched_ret);
		...
		struct ion_special_scene_pool_info_data *scene_pool_info)
{
	if (flag == F_WAKE_UP) {
		...
		scene_pool->worker_mask = (unsigned)scene_pool_info->worker_mask;
		atomic_set(&scene_pool->total_watermark, scene_pool_info->water_mark);
		cancel_delayed_work((struct delayed_work *)(&scene_pool->autofree_delayed_work));
		...
		if (scene_pool_info->autostop_timeout > MAX_SPECIAL_SCENE_TIMEOUT)
			scene_pool_info->autostop_timeout = MAX_SPECIAL_SCENE_TIMEOUT;
		scene_pool->timeout_hz = scene_pool_info->autostop_timeout * HZ;
		if (scene_pool->timeout_hz > 0) {
			ret = queue_delayed_work(system_wq,(struct delayed_work *)(&scene_pool->autofree_delayed_work),	scene_pool->timeout_hz);
			if (!ret)
				mod_delayed_work(system_wq,(struct delayed_work *)(&scene_pool->autofree_delayed_work),	scene_pool->timeout_hz);
		}
		...
		pr_info("Start watermark=%d, worker=0x%x, timeout=%d S\n",
			...
			scene_pool_info->worker_mask,
			scene_pool_info->autostop_timeout);
	} else if (flag == F_FORCE_STOP || (flag == F_WAKEUP_AUTOFREE && !scene_pool_info->autostop_timeout)) {
...
void mtk_ion_pool_wakeup_notify(int inflag, int water_mark, int timeout) {
	int worker_mask = SPECIAL_SCENE_FILL_POOL_WORKER;
	ion_scene_pool_wakeup_process(inflag, water_mark, worker_mask, timeout);
}

void ion_scene_pool_wakeup_process(int inflag, int water_mark, int worker_mask, int timeout)
{
	struct ion_scene_pool *scene_pool = (struct ion_scene_pool *)ion_get_scene_pool();
	enum scene_pool_wait_flags flag = (enum scene_pool_wait_flags)inflag;

	if (!scene_pool || flag == F_NONE) {
		pr_err("ion_scene_pool_wakeup_process null args");
		return;
	}

	struct ion_special_scene_pool_info_data scene_pool_info ={0};
	scene_pool_info.water_mark = water_mark;
	scene_pool_info.worker_mask = worker_mask;
	scene_pool_info.autostop_timeout = timeout;
...
void ion_scene_pool_destroy(struct ion_scene_pool *pool)
{
	...
	ion_scene_destroy_pools(pool->pools);
	kfree(pool);
}
...
struct ion_scene_pool *ion_scene_pool_create(void)
{
	...
	if (ion_scene_create_pools(scene_pool->pools, false))
#else
	if (ion_scene_create_pools(scene_pool->pools, false))
#endif
		goto free_heap;
...
destroy_pools:
	ion_scene_destroy_pools(scene_pool->pools);
