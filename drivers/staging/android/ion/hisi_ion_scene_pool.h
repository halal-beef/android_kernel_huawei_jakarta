...
#endif
	if (!pool) {
		pr_err("ion_scene_pool_can_allocate null pool\n");
		ret = false;
	}
...
void ion_scene_pool_debug_show_total(struct seq_file *s, struct ion_scene_pool *scene_pool);
int ion_scene_pool_shrink(struct ion_scene_pool *scene_pool, struct ion_page_pool *pool, gfp_t gfp_mask, int nr_to_scan);
void ion_scene_pool_destroy(struct ion_scene_pool *pool);
void ion_scene_update_watermark(void *p, int delta);
void ion_scene_pool_wakeup_process(int flag,	int water_mark, int water_mask, int timeout);
unsigned long ion_scene_pool_allocate_pages(struct ion_scene_pool *pool, unsigned long size, int *count, struct list_head *page_list);
extern void *ion_get_scene_pool(void);
...
#define ION_SCENE_POOL_TRY_ALLOC_ENTER(in_pool,		\
					...
					begin)		\
{	\
	if (in_pool && in_pool->in_special_scene && strstr(current->comm, SCENE_POOL_ALLOW_THREAD)) {	\
...
\
		...
		if (delta >= 1000 || allocated)		\
			pr_info("%s, special scene alloc %d[%d] kb"	\
				" consume %ld us\n",			\
				__func__,				\
				(ALIGN(size, PAGE_SIZE) >> PAGE_SHIFT)*4,	\
				(allocated >> PAGE_SHIFT)*4,		\
				delta);					\
	}	\
