...
#define LOCK_TRY_COUNT 100000
#define DELAY_TIME 10
static int sdhci_trylock_hostlock(struct msdc_host *host, unsigned long *flags)
{
	int locked;
	unsigned int trycount = LOCK_TRY_COUNT;
 ...
 */
int msdc_send_command_direct(struct mmc_host *mmc, struct mmc_request *mrq)
{
	int ret;
	unsigned long flags;
	struct msdc_host *host;
	...
	ret = msdc_do_command(host, mrq->cmd, CMD_TIMEOUT);
	if (ret)
		pr_err("%s: msdc_do_command fail\n", __func__);
...
#ifdef CONFIG_HUAWEI_MMC_FLUSH_REDUCE
	.send_cmd_direct = msdc_send_command_direct,
#endif
};
