...
static void huawei_blk_flush_ctrl(struct mmc_blk_data *md,
	struct mmc_card *card)
{
	if (card->host->mmc_flush_reduce_enable)
		blk_queue_direct_flush_register(md->queue.queue,
			mmc_blk_direct_flush);
	else
		blk_queue_direct_flush_register(md->queue.queue, NULL);
...
void part_curr_update(struct mmc_card *card)
{
	...
	part_config = card->ext_csd.part_config;
	part_config &= EXT_CSD_PART_CONFIG_ACC_MASK;
	pr_notice("ffu restart part_config = %d, part_curr = %d, part_type = %d\n",
